...
public class A2MainRunner {
	...
	public static void main(String[] args) {
		 ...
		 */
		for (int i = 1; i <= 6; i++) {
			String file = "tests" + System.getProperty("file.separator")
					+ "Test" + i + ".javax";
			try {
				A2Compiler.compile(new File(file));
				System.out.println(file + " ... OK");
			} catch (ParseException e) {
				System.err.println(file + " Parser exception... "
						+ e.getMessage());
				e.printStackTrace();
			} catch (A2SemanticsException e) {
				System.err.println(file + " Semantics exception... "
						+ e.getMessage());
				e.printStackTrace();
		...
		try {
			A2Compiler.compile(new File("src"
					+ System.getProperty("file.separator") + "se701"
